...
class ClassificationTransformer(nn.Module):
    ...
    def embed(self, inputs):
        ...
        embeddings += self.position_embed(torch.arange(inputs.shape[1]))
    ...
    def multi_head_attention(self, inputs):
        ...
        Q1 = self.q1(inputs)
        V1 = self.v1(inputs)
        att1 = torch.bmm(self.softmax(torch.bmm(Q1, K1.transpose(1,2)) / np.sqrt(self.dim_k)), V1)
        
        # Attention Head 2
        K2 = self.k2(inputs)
        Q2 = self.q2(inputs)
        V2 = self.v2(inputs)
        att2 = torch.bmm(self.softmax(torch.bmm(Q2, K2.transpose(1,2)) / np.sqrt(self.dim_k)), V2) 
        
        outputs = self.attention_head_projection(torch.cat((att1,att2), dim=2))
    ...
    def feedforward_layer(self, inputs):
        ...
        relu_out = self.relu(out1)
        out2 = self.fc2(relu_out)
        outputs = self.norm_mh(out2+inputs)
