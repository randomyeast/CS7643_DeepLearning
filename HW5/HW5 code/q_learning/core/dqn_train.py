...
class DQNTrain(QNTrain):
    ...
    def process_state(self, state):
        ...
        state = torch.div(state, self.config.high).to(self.device)
        #####################################################################
        #                             END OF YOUR CODE                      #
        #####################################################################
        return state
    ...
    def forward_loss(
        ...
        qsamp = reward.clone()
        qsamp += (done_mask)*self.config.gamma*self.target_q_net(next_state).max(1).values

        q_sa = self.q_net(state)
        q_sa = torch.gather(q_sa, 1, action.view(-1,1)).squeeze(1)

        loss = ((qsamp - q_sa)**2).sum()
        #####################################################################
        #                             END OF YOUR CODE                      #
        #####################################################################
        return loss
    ...
    def update_step(self, t, replay_buffer, lr):
        ...
        sp_batch = self.process_state(sp_batch).to(self.device)
        done_mask_batch = torch.FloatTensor(done_mask_batch).to(self.device)
